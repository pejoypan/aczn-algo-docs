...
在数粒线中，药品通过振动盘或输送轨道进入计数通道，每个通道（slat）应该容纳一个药片。Slat Mode 可以智能地区分以下情况：
- 有槽位且装有药片 → 正常
- 有槽位但未装药片 → 漏装缺陷（Missing）
- 无槽位且未装药片 → 空位（Blank，正常现象）
- 无槽位但装有药片 → 多余药片缺陷（Extra_Cavity）

## 2. 与传统模式的区别

|        | 普通模式 | Slat Mode |
|--------|----------|-----------|
| 无药片处理 | 一律判为 N/A | 有槽位 → N/A<br />无槽位 → Blank（正常） |
...
| 槽位状态 | 药片状态 | 判定结果 | 说明 |
|----------|----------|----------|------|
| 有槽位 | 有药片 | 正常检测 | 进入常规缺陷检测流程 |
| 有槽位 | 无药片 | **Missing** | 漏装缺陷 |
| 无槽位 | 无药片 | **Blank** | 空位，正常现象 |
| 无槽位 | 有药片 | **Extra_Cavity** | 多余药片，异常 |
...
| 缺陷类型 | 含义 | 触发条件 |
|----------|------|----------|
| **BLANK** | 空位 | 无槽位位置未检测到药片（正常现象） |
| **Extra_Cavity** | 多余药片 | 无槽位位置检测到药片 |
| **Missing** | 漏装 | 有槽位位置未检测到药片 |
